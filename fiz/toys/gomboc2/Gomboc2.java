 ...
 * potential V(r) = cos(4*pi*r*exp(-r^2))/r
 * @version 05/10/2006
 * @since 05/07/2006
 * @author Peter Csizmadia
 */
public class Gomboc2 extends JApplet {
	...
	double phi = 4*Math.PI*r*Math.exp(-r*r);
	return Math.cos(phi)/r;
    }

    private static double forceExact(double r) {
	double w = 4*Math.PI*Math.exp(-r*r);
	double phi = w*r;
	return -(Math.sin(phi)*w*(1-2*r*r) + Math.cos(phi)/r)/(r*r);
    }
...
/*    private void step(double dt) {
	...
	f.setTitle("G\u00f6mb\u00f6cband\u00e1k");
	final JApplet applet = new Gomboc2();
